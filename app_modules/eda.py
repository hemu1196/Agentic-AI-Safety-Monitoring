...
def render_eda_page(df):
    ...
    <div style="margin-bottom: 20px;">
        <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; color: #f8fafc; margin: 0;">🔍 Exploratory Analytics Studio</h2>
        <p style="color: #94a3b8; font-size: 0.95rem;">Multi-dimensional inspection, feature correlations, and 3D terrain mapping.</p>
    </div>
    ...
    # Filter Panel
    with st.expander("🛠️ Dynamic Telemetry Filter Panel", expanded=True):
        ...
        with f_col2:
            if "temperature" in df.columns:
                ...
                selected_temp = st.slider("Temperature (°C)", float(round(min_t, 1)), float(round(max_t, 1)), (float(round(min_t, 1)), float(round(max_t, 1))))
            else:
                selected_temp = None

        with f_col3:
            if "material_shortage_alert" in df.columns:
                shortage_filter = st.multiselect("Shortage Alert", options=[0, 1], default=[0, 1], format_func=lambda x: "Shortage Alert (1)" if x == 1 else "Normal (0)")
            else:
        ...
        with f_col4:
            if "optimization_suggestion" in df.columns:
                ...
                selected_opts = st.multiselect("Optimization Strategy", options=opts, default=opts)
            else:
                selected_opts = None
    ...
    st.caption(f"⚡ Displaying **{len(filtered_df):,}** filtered records ({len(filtered_df)/len(df)*100:.1f}% of total).")

    tab1, tab2, tab3, tab4 = st.tabs(["🔥 Feature Correlation Matrix", "📊 Custom Feature Scatter", "🌐 3D Risk Terrain", "📋 Summary Statistics"])
    ...
    with tab1:
        ...
        if len(num_cols) > 1:
            ...
            fig_corr = px.imshow(
                ...
                title="Correlation Heatmap Across Telemetry Predictors",
                aspect="auto"
            )
            apply_plotly_theme(fig_corr, height=520)
    ...
    with tab2:
        ...
        with c_col3:
            ...
            color_by = st.selectbox("Color Segment By", options=["None"] + cat_cols, index=0)
        ...
        fig_scatter = px.scatter(
            ...
            opacity=0.8,
            title=f"Bivariate Analysis: {y_axis} vs {x_axis}",
            color_discrete_sequence=px.colors.qualitative.Bold
        )
        apply_plotly_theme(fig_scatter, height=450)
        st.plotly_chart(fig_scatter, use_container_width=True)

    with tab3:
        z_axis = st.selectbox("Z-Axis Feature (Height)", options=num_cols, index=num_cols.index("risk_score") if "risk_score" in num_cols else 0)
        
        sample_3d = filtered_df.sample(min(1200, len(filtered_df)))
        fig_3d = px.scatter_3d(
            ...
            title=f"3D Topography: Temperature vs Humidity vs {z_axis}"
        )
        apply_plotly_theme(fig_3d, height=580)
